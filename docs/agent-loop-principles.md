...
- 从 review job 生成结构化修复 Prompt。
- 修复 Prompt 重点交代评审上下文和评审结果，不重复携带源输入预览；源线程已有上下文由源 Agent 自己读取。
- 支持“复制并打开源线程”，但不覆盖剪贴板里的修复 Prompt。
- 记录轻量 fix loop 状态：尚未处理、已复制 Prompt、已回源线程、已处理、不采纳。
- 仍由用户把 Prompt 粘回源 Agent，并由用户确认是否执行。
