...
src/search-index.mjs    本地 SQLite FTS 搜索索引、线程搜索和项目历史
...
- `GET /api/search`
  - 查询本地搜索索引；支持来源、状态、项目、归档、Sub Agent、Automation 和分页参数。
- `GET /api/search/status`
  - 返回搜索索引可用状态、索引时间和线程数量。
- `POST /api/search/reindex`
  - 重建本地搜索索引；使用较大的历史读取窗口，不阻塞常规 dashboard 刷新路径。
...
- 搜索索引、搜索 API、项目历史、artifact 解析和本地文件预览 / 打开。
