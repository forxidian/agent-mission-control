...
Agent Mission Control 是一个本地只读任务控制台，用来集中查看 Codex、OpenCode、Claude Code/Cowork 等 Agent 线程、项目分布、token 用量、quota、运行状态、待处理事项和桌面提醒，并能一键跳转或恢复到对应线程继续工作。
...
- 支持复制 resume / open 命令。
- 支持本地桌面提醒，提醒文案隐私化，只显示“Codex 有新进展待处理”，不泄露线程正文。
...
src/notifications.mjs   通知中心、桌面提醒、通知持久化、软提醒过期
...
- `~/.agent-mission-control/notifications.json`
  - 通知状态、已观察 completion signal、桌面提醒设置。
...
- 软提醒：
  ...
  - 用来触发一次桌面提醒，但只短暂保留约 60 秒，不长期占用待处理。
- macOS 通知文案固定隐私化：
  - title：`Codex 有新进展待处理`
  - message：空字符串
...
- 旧问题：通知 Push 泄露具体正文。
  - 现在：所有桌面提醒都只显示“Codex 有新进展待处理”。
- 旧问题：系统通知会出声音。
  - 已在 macOS 通知设置侧调整为无声音；代码里也不主动设置声音。
- 旧问题：同一线程标题显示成不对应的文字。
...
- `POST /api/notifications`
  - 刷新通知并触发 pending desktop notification。
